(function () {
  ...
  function getItems(payload) {
    ...
    if (Array.isArray(payload && payload.data && payload.data.traders)) return payload.data.traders;
  ...
  function normalizeRecord(record, index) {
    var name = pick(record, ['identity.name', 'name', 'handle', 'username', 'displayName', 'walletName', 'profile.name']) || 'KOL Wallet';
    var address = pick(record, ['wallet', 'address', 'walletAddress', 'owner', 'profile.wallet', 'profile.address']);
    var profileUrl = pick(record, ['profileUrl', 'url', 'sourceUrl', 'profile.url', 'identity.twitter']);

    if (profileUrl && String(profileUrl).charAt(0) === '@') {
      profileUrl = 'https://x.com/' + String(profileUrl).slice(1);
    }
    ...
    return {
      ...
      totalPnl: pick(record, ['pnl.total', 'totalPnl', 'totalPnL', 'pnl', 'profit', 'totalProfit']),
      realizedPnl: pick(record, ['pnl.realized', 'realizedPnl', 'realizedPnL', 'realized', 'realizedProfit']),
      winRate: pick(record, ['winRate', 'winrate', 'wins', 'performance.winRate']),
      trades: pick(record, ['counts.trades', 'trades', 'tradeCount', 'totalTrades']),
